...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $values['captcha'] = htmlspecialchars($captcha_input);

    if (strlen($login) < 3) $errors['login'] = "Логин должен быть не менее 3 символов.";
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) $errors['email'] = "Некорректный email.";
    if (strlen($password_raw) < 6) $errors['password'] = "Пароль должен содержать не менее 6 символов.";
    if ($captcha_input != $_SESSION['captcha']) $errors['captcha'] = "Неверный код с картинки.";
    ...
    if (empty($errors)) {
        ...
        $stmt->close(); //варианты обработки запроса
    }
}
?>
<!DOCTYPE html>
...
<div class="reg-form">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <input type="text" name="login" id="login" value="<?= $values['login'] ?>" required>
        <?php if (!empty($errors['login'])) echo '<div class="error-message">' . $errors['login'] . '</div>'; ?>
        ...
        <input type="text" name="captcha" id="captcha" value="<?= $values['captcha'] ?>" required>
        <?php if (!empty($errors['captcha'])) echo '<div class="error-message">' . $errors['captcha'] . '</div>'; ?>
